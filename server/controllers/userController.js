...
import { CourseProgrss } from "../models/CourseProgress.js";
...
// PURCHASE COURSES THROUGH PAYMENT GATEWAY
  ...
  try {
    ...
    if (!lectureId) {
      return res.status(400).json({
        success: false,
        message: "Invalid request: Missing lectureId",
      });
    }

    const progressData = await CourseProgrss.findOne({ userId, courseId });
...
export const addUserRatimg = async (req, res) => {
  const userId = req.auth.userId;
  ...
  try {
    ...
    if (!user || !user.enrolledCourses.includes(courseId)) {
      return res.json({
        success: false,
        message: "user has not purchase the course",
      });
    }
    ...
    console.log("ratings---------------" + course.courseRating);
